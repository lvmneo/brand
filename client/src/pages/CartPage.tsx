...
export default function CartPage() {
  ...
  const handleMockCardPayment = async () => {
    ...
    if (digitsOnly.length !== 16) {
      showToast('Номер карты должен содержать 16 цифр', 'error')
      return
    }

    if (!cardHolder.trim()) {
      showToast('Укажи имя держателя карты', 'error')
      return
    }

    if (!/^\d{2}\/\d{2}$/.test(expiry.trim())) {
      showToast('Укажи срок действия в формате MM/YY', 'error')
      return
    }

    if (!/^\d{3}$/.test(cvv.trim())) {
      showToast('CVV должен содержать 3 цифры', 'error')
      return
    }
    ...
    try {
      ...
      showToast('Demo-оплата картой подтверждена')
    } finally {
  ...
  const handleMockSbpPayment = async () => {
    if (!sbpBank.trim()) {
      showToast('Выбери банк для оплаты по СБП', 'error')
      return
    }
    ...
    try {
      ...
      showToast('Demo-оплата по СБП подтверждена')
    } finally {
  ...
  const handleCheckout = async () => {
    ...
    if (!recipientName.trim() || !phone.trim() || !city.trim()) {
      showToast('Заполни имя, телефон и город', 'error')
      return
    }

    if (deliveryMethod === 'COURIER' && !address.trim()) {
      showToast('Укажи адрес доставки', 'error')
      return
    }
